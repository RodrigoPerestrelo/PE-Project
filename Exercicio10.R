# Instalação do pacote pracma se não estiver instalado
if (!require(pracma)) {
...
# Dividindo o suporte em k classes
amplitude <- (b - a) / k
limites_classes <- seq(a, b, by = amplitude)

# Calculando as frequências observadas
frequencias_obs <- table(cut(observacoes, breaks = limites_classes, include.lowest = TRUE))

# Função de distribuição acumulada para a distribuição triangular simétrica
...
prob <- sapply(1:(length(limites_classes) - 1), function(i) {
  cdf_triangular(limites_classes[i + 1], a, b) - cdf_triangular(limites_classes[i], a, b)
})
n <- length(observacoes)
frequencias_esp <- prob * n

# Calculando o valor do teste qui-quadrado
qui_quadrado <- sum((as.numeric(frequencias_obs) - frequencias_esp)^2 / frequencias_esp)

# Graus de liberdade
g_liberdade <- length(frequencias_obs) - 1

# Valor-p
valor_p <- 1 - pchisq(qui_quadrado, g_liberdade)

# Exibindo o valor-p arredondado a 4 casas decimais
round(valor_p, 4)
